...
const AdvScreen = () => {
  ...
  const filteredAdvertisements = advertisements.filter(ad => ad.id === id);
...
<View style={{ flex: 1, alignItems: 'flex-end', justifyContent: 'center', backgroundColor: "#FFFDED"}}>
      <View style={{width:"100%", height: "100%"}}>
         <Text style={styles.title}>{filteredAdvertisements[0]?.title}</Text>
         <ScrollView contentContainerStyle={{flex: 1, width: '100%'}}>
        {filteredAdvertisements.map(ad => (
              <Image source={{ uri: ad.photoLink }} style={{width: '100%', height: 240, marginTop: 12}} />
        ))}
         <Text style={styles.price}>{filteredAdvertisements[0]?.price} ₽/ч.</Text>
      <MapView style={styles.map} />
         <Text style={styles.description}>Просторное помещение с высокими потолками и изысканной подсветкой создает атмосферу уюта и праздничности. </Text>
         <AppButton title="Позвонить" backgroundColor="#FF8787" onPress={() => opencall(filteredAdvertisements[0]?.numberphone)} />
          <Text style={{color: '#FF8787', fontSize: 16, textAlign: 'center', fontWeight: 'bold'}}
        onPress={like}>Добавить в Избранное  ♥</Text>
      </ScrollView>
      </View>
